...
<p><b>FVG (Fair Value Gap)</b> = zone de déséquilibre sur 3 bougies <b>4 h</b>, d'une taille minimale visible (≥ 0,5 × ATR H4). Le plan reste affiché tant que la zone n'est pas comblée.</p>
<p><b>Casse H4</b> = confirmation par clôture au-delà du plus haut/bas récent.</p>
<p><b>Cycle de vie d'un setup</b> : ACTIF · INVALIDÉ (stop touché) · PARTI (prix au-delà de 1R sans toi) · TERMINÉ (TP1 touché). Un setup périmé n'est plus proposé.</p>
<p><b>Stop</b> = bas de zone FVG − 0,5 × ATR H4 (ou niveau cassé ± 2 × ATR H4). <b>TP1/TP2</b> = triés du plus proche au plus loin.</p>
<p><b>IV</b> = agitation attendue · <b>Skew</b> = prix de la peur · <b>Max pain</b> = aimant de fin de semaine · <b>GEX</b> = amortisseurs du marché · <b>COT</b> = positionnement des “gros”.</p>
...
def conseil_systeme(r, reg, metal, gex=None, cot=None, opt=None):
    ...
    if r["scenario"] == "CASSE":
        ...
        t.append(f"Zone FVG 4 h visible : entrée limite à <b>{r['entree']:.2f}</b> si le prix (re)descend dans la zone — le plan reste affiché tant que la zone n'est pas comblée.")
    t.append(f"Stop loss obligatoire à <b>{r['stop']:.2f}</b> (bas de zone − 0,5 × ATR H4), jamais élargi.")
    if r["alerte"]:
...
if st.button("🔄 Générer l'analyse du jour", type="primary"):
    with st.spinner("Analyse en cours (1-2 minutes)..."):
        ...
        cb_html = ""
        if r_semaine <= -2.0:
            cb_html = "<div class='alert'>🛑 <b>CIRCUIT BREAKER</b> : −2 % atteint cette semaine → pause jusqu'à lundi. Le capital d'abord.</div>"
        ...
        Risque modulé : <b>{risque_pct} %</b> par trade | Cœur : Dow Daily + FVG 4 h (taille mini) / Casse H4</div>
        {resume_html}
        {metaux_html}
        {CONCEPTS}
        {cartes}
