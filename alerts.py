# ===== alerts.py v5 — sentinelle H4 (cassures + FVG taille mini) via ntfy.sh =====
import requests
    ...
    try:
        ...
        if td == 1:
            ...
            if last["Close"] > lvl and prev["Close"] <= lvl:
                msgs.append(f"{nom} : LONG confirmé (cassure H4 au-dessus de {lvl:.2f}). Stop : {lvl - 2*atr:.2f}. Vérifie le rapport avant décision.")
            for (zf, ty, top, bot) in fvg_zones(H4):
                ...
                if last["Low"] <= top and prev["Low"] > top:
                    msgs.append(f"{nom} : repli dans la zone FVG 4 h → entrée limite {top:.2f}, stop {bot - 0.5*atr:.2f}. Vérifie le rapport avant décision.")
        elif td == -1:
            lvl = H4["LL20"].shift(1).iloc[i]
            if last["Close"] < lvl and prev["Close"] >= lvl:
                msgs.append(f"{nom} : SHORT confirmé (cassure H4 en dessous de {lvl:.2f}). Stop : {lvl + 2*atr:.2f}. Vérifie le rapport avant décision.")
            for (zf, ty, top, bot) in fvg_zones(H4):
                if ty != "S" or zf > i or zf < i-40: continue
                if (top - bot) < 0.5 * atr: continue
                if (H4["Close"].iloc[zf:i] > top).any(): continue
                if last["High"] >= bot and prev["High"] < bot:
                    msgs.append(f"{nom} : repli dans la zone FVG 4 h → entrée limite {bot:.2f}, stop {top + 0.5*atr:.2f}. Vérifie le rapport avant décision.")
